...
function createTCEQPushData(aqsid, data) {
	...
	try {
		...
		throw new Error('Could not write TCEQ push file.', `Could not write TCEQ push file. Error: ${error}`);
	}
}


// performs the creation of 5 minute aggregate data points 
function perform5minAggregat(siteId, startEpoch, endEpoch, callback) {
	...
	db.aggregate(collectionNames.LiveData, pipeline, { allowDiskUse: true }, function(err, res, callbackAggregate) {
		let insertIntoAggrData5min = [];
		...
		res.forEach((e) => {
			...
			if (!insertIntoAggrData5min.some((ele) => ele._id === subObj._id)) {
				insertIntoAggrData5min.push(subObj);
			}
		}).then(() => {

			// Realized that importOld doesn't need as many checks as liveImport does. An insertMany does the job just as well
			if (insertIntoAggrData5min.length) {
				db.insertMany(collectionNames.AggrData, insertIntoAggrData5min, { ordered: false, supressErrors: true }, callback);
			}
...
function perform5minAggregatBC2(siteId, startEpoch, endEpoch, callback) {
	// create temp collection as placeholder for aggreagation results
	const pipeline = [
		{
			$match: {
	...
	db.aggregate(collectionNames.LiveData, pipeline, { connectTimeoutMS: 300000, socketTimeoutMS: 300000, allowDiskUse: true }, function(err, res) {
		let insertIntoAggrData5min = [];
		...
		res.forEach((e) => {
			...
			if (previous_ids.find(ele => ele === subObj._id)) {
				return;
			} 
			previous_ids.push(subObj._id);
...
async function callToBulkUpdate(allObjects, path, site, startEpoch, endEpoch, daqFactory) {
	...
	// supressErrors is my own option. I supress error message that probably will occur so console isn't ugly filled with duplicate key error messages
	db.insertMany(collectionNames.LiveData, allObjects, { ordered: false, supressErrors: true }, function() {
		...
		if (daqFactory) {
			...
			if (site.siteGroup.includes("BC2")) {
				perform5minAggregatBC2(site.AQSID, startAggrEpoch, endAggrEpoch, () => { store.set('aggregationFunctionsRunning', false) });
			} else {
				try {
					perform5minAggregat(site.AQSID, startAggrEpoch, endAggrEpoch, () => { store.set('aggregationFunctionsRunning', false) });
				} catch (err) {
					logger.error(err);
					store.set('aggregationFunctionsRunning', false);
				}
			}
		} else {
			store.set('aggregationFunctionsRunning', false);
...
function batchLiveDataUpsert(parsedLines, path) {
	if (parsedLines === undefined || parsedLines[0] === undefined) {
		store.set('aggregationFunctionsRunning', false);
		return;
	}
...
function batchMetDataUpsert(parsedLines, path) {
	if (parsedLines === undefined || parsedLines[0] === undefined) {
		store.set('aggregationFunctionsRunning', false);
		return;
	}
	...
	const pathArray = path.split(pathModule.sep);
	const parentDir = pathArray[pathArray.length - 2];
	// Old metoer code: const site = LiveSites.findOne({ incoming: parentDir });
...
function batchTapDataUpsert(parsedLines, path) {
	if (parsedLines === undefined || parsedLines[0] === undefined) {
		store.set('aggregationFunctionsRunning', false);
		return;
	}
	store('aggregationFunctionsRunning', true);
	// find the site information using the location of the file that is being read
	const pathArray = path.split(pathModule.sep);
	const parentDir = pathArray[pathArray.length - 2];
	...
	if (site.AQSID) {
		...
		// create objects from parsed lines
		const allObjects = [];
		// Sometimes 2 objects on the same epoch appear. For now, delete the second one
		let previousEpoch = 0;
		for (let k = 0; k < parsedLines.length; k++) {
			const singleObj = {};
			singleObj.subTypes = {};
...
async function initialize(obj) {
	store.set('aggregationFunctionsRunning', false);
	// Sometimes it is best just to send in the site object and store it that way instead of requerying it
	if (obj.site === undefined) {
		site = findOne(collectionNames.LiveSites, 
			{ 
				$or: [
					{ AQSID: obj.AQSID },
		...
		site = JSON.parse(JSON.stringify(obj.site));
	}
}

module.exports = { makeObj, readFile, createTCEQPushData, batchTapDataUpsert, batchMetDataUpsert, batchLiveDataUpsert, callToBulkUpdate, initialize, perform5minAggregat, perform5minAggregatBC2 };
