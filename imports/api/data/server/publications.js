...
// publications for all composite plots - based on site group
Meteor.publish('compositeGroupDataSeries', function (startEpoch, endEpoch, siteGroupName) {
  const subscription = this;
  const pollCompData = {};

  const selectedGroup = LiveSites.find({ siteGroup: siteGroupName }).fetch();
  let selectedAQSID = [];
  if (siteGroupName === 'UHCampus') { // pre-define list of sites for UH Campus
    selectedAQSID = ['482010695', '482011611'];
  } else {
  ...
  // wait for results from aggregation
        ...
        _.each(line._id.subTypes, (data) => { // Instrument, HPM60 etc.
          _.each(data, (points, origMeasurement) => { // sub is the array with metric/val pairs as subarrays, measurement, WS etc.
            ...
            if (_.last(points).val === 1) { // get all measurements where flag == 1
              let datapoint = {};
              // HNET special treatment for precipitation using sum instead of avg
              if (measurement.indexOf('Precip') >= 0 || points.length < 5) {
                datapoint = {
                  x: epoch * 1000, // milliseconds
                  y: parseFloat(points[0].val) // sum or calc
                };
              } else {
                datapoint = {
                  x: epoch * 1000, // milliseconds
                  y: parseFloat(points[1].val) // average
                };
              }
